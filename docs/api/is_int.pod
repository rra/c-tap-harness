...
is_bool, is_int, is_double, is_string, is_hex, is_blob - Check values for a TAP
test
...
int B<is_bool>(int I<wanted>, int I<seen>, const char *I<format>, ...);
...
int B<is_blob>(const void *I<wanted>, const void *I<seen>, size_t I<len>,
	   const char *I<format>, ...);
...
is_double() compares two doubles, is_string() compares two strings, and
is_hex() compares two unsigned longs.  The advantage of these functions
over ok() is that, if the values don't match, the expected and seen values
are reported as diagnostics to make it easier to understand the test
failure.  Note that, although is_bool() takes parameters of type int, both
values are evaluated for their truth value not for their numeric value.
(In other words, C<is_bool(1, 4)> is a passing test.)

is_blob() takes two pointers and compares I<len> bytes of data from them, any
bytes which do not match will be reported and fail the test.
