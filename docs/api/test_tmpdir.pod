...
C_TAP_BUILD environment variable, or under the current directory if
C_TAP_BUILD is not set.
...
the environment variable C_TAP_BUILD is not set or is relative itself.
