...
B<runtests> with the B<-l> option.  Either way, each test must be an
executable ending in either C<-t> or C<.t> and its name given to
B<runtests> with the extension removed.  The program name should normally
be given as a relative path (see below for source and build directory
handling).  B<runtests> will run each test program in the order listed and
