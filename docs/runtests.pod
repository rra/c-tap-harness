...
reimplementation executables API
...
B<runtests> [B<-h>] [B<-b> I<builddir>] [B<-s> I<srcdir>] I<test> ...

B<runtests> [B<-b> I<builddir>] [B<-s> I<srcdir>] B<-l> I<test-list>
...
B<runtests> with the B<-l> option.  Either way, each test name should be a
path to an executable, possibly with a trailing C<-t> or C<.t> removed.
The path should normally be given as relative to the source or build
directory; see below for source and build directory handling.  B<runtests>
will run each test program in the order listed and summarize the results.
...
Copyright 2009, 2010, 2011, 2013 Russ Allbery <rra@stanford.edu>
