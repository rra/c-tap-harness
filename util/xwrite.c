/* $Id$
   ...
   it left off until the full amount of the data is written.  The write is
   also repeated if it failed with EINTR.  The write will be aborted after
   10 successive writes with no forward progress.
   ...
   functions will return -1 and the number of bytes actually written will be
   lost. */
