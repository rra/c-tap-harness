...
where <last> is the number of the last test.  C<1..> may be omitted.  For
each test, the script must output a line in one of these three formats:
...
protocol all other output should go to standard error and begin with a
C<#> sign.
